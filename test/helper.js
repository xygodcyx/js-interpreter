...
import Eval, { SingleObjectInstance } from '../src/evaluator.js';
import {
    BaseObject,
    BooleanObj,
    IntegerObj,
...
export function testIntegerLiteral(t, exp, expected) {
    if (!exp || !(exp instanceof IntegerLiteral)) {
        t.error(`exp is not IntegerLiteral. got=${exp?.constructor?.name}`);
        return false;
    }
    ...
    if (exp.tokenLiteral() !== expected.toString()) {
        t.error(`exp.tokenLiteral not ${expected}. got=${exp.tokenLiteral()}`);
        return false;
    }
...
export function testIdentifier(t, exp, value) {
    ...
    if (!(exp instanceof Identifier)) {
        t.error(`exp 不是 *ast.Identifier. 实际类型=${exp.constructor.name}`);
        return false;
    }
    ...
    if (ident.value !== value) {
        t.error(`ident.Value 不是 ${value}. 实际值=${ident.value}`);
        return false;
    }

    // 检查 TokenLiteral 值
    if (ident.tokenLiteral() !== value) {
        t.error(`ident.TokenLiteral 不是 ${value}. 实际值=${ident.tokenLiteral()}`);
        return false;
    }
 ...
 */
export function testInfixExpression(t, exp, left, operator, right) {
    // 类型检查
    if (!(exp instanceof InfixExpression)) {
        t.error(`exp 不是 InfixExpression. 实际类型= ${exp?.constructor?.name}`);
        return false;
    }
    ...
    if (exp.operator !== operator) {
        t.error(`运算符不是 '${operator}'. 实际值= '${exp.operator}'`);
        return false;
    }
...
export function testBooleanLiteral(t, exp, value) {
    if (!(exp instanceof BooleanLiteral)) {
        t.error(`exp 不是 BooleanLiteral. 实际类型=${exp.constructor.name}`);
        return false;
    }
    if (exp.value !== value) {
    ...
    if (exp.tokenLiteral() !== String(value)) {
        t.error(`tokenLiteral 不是 ${value}. 实际值=${exp.tokenLiteral()}`);
        return false;
    }
...
export function testIntegerObject(obj, expected) {
    if (!(obj instanceof IntegerObj)) {
        console.error(
            `object is not Integer. got=${obj?.constructor?.name} (${JSON.stringify(obj)})`
        );
        return false;
    }
    if (obj.value !== expected) {
        console.error(`object has wrong value. got=${obj.value}, want=${expected}`);
        return false;
    }
    return true;
}
...
export function testStringObject(obj, expected) {
    if (!(obj instanceof StringObj)) {
        console.error(
            `object is not String. got=${obj?.constructor?.name} (${JSON.stringify(obj)})`
        );
        return false;
    }
    if (obj.value !== expected) {
...
export function testBooleanObject(obj, expected) {
    if (!(obj instanceof BooleanObj)) {
        console.error(
            `object is not Boolean. got=${obj?.constructor?.name} (${JSON.stringify(obj)})`
        );
        return false;
    }
    if (obj.value !== expected) {
...
export function testNotValueObject(obj, expected) {
    ...
    console.error(`object is not NotValueObject. got=${obj?.constructor?.name} (${JSON.stringify(obj)})`);
}
...
export function testNullObject(obj) {
    if (obj !== SingleObjectInstance.NULL) {
        console.error(`object is not NULL. got=${obj?.constructor?.name} (${JSON.stringify(obj)})`);
        return false;
    }
    return true;
}
...
export function testVoidObject(obj) {
    if (obj !== SingleObjectInstance.VOID) {
        console.error(`object is not VOID. got=${obj?.constructor?.name} (${JSON.stringify(obj)})`);
        return false;
    }
    return true;
}
...
export function testUndefinedObject(obj) {
    if (obj !== SingleObjectInstance.UNDEFINED) {
        console.error(
            `object is not UNDEFINED. got=${obj?.constructor?.name} (${JSON.stringify(obj)})`
        );
        return false;
    }
    return true;
