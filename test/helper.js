...
import { BaseObject, BooleanObj, IntegerObj } from '../src/object.js';
...
export function testIntegerObject(obj, expected) {
  if (!(obj instanceof IntegerObj)) {
    console.error(`object is not Integer. got=${obj?.constructor?.name} (${JSON.stringify(obj)})`);
    return false;
  }
...
export function testBooleanObject(obj, expected) {
  if (!(obj instanceof BooleanObj)) {
    console.error(`object is not Boolean. got=${obj?.constructor?.name} (${JSON.stringify(obj)})`);
    return false;
  }
  if (obj.value !== expected) {
    console.error(`object has wrong value. got=${obj.value}, want=${expected}`);
